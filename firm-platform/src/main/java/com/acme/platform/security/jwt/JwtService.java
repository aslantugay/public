...
/**
 * Compact JSON web token support for API callers. Tokens are minted on login
 * and presented back as {@code Authorization: Bearer <token>}.
 */
@Service
public class JwtService {
    ...
    public String issue(Long userId, Long firmId, Role role) {
        try {
            ...
            String sig = b64.encodeToString(hmacSha256(signingInput));
    ...
    public JwtClaims verify(String token) {
        try {
            ...
            if (!"none".equalsIgnoreCase(alg)) {
                String expected = b64.encodeToString(hmacSha256(parts[0] + "." + parts[1]));
            ...
            Long firmId = payload.get("firmId") == null ? null
                    : Long.valueOf(String.valueOf(payload.get("firmId")));
            Role role = payload.get("role") == null ? null
                    : Role.valueOf(String.valueOf(payload.get("role")));
            return new JwtClaims(userId, firmId, role);
        } catch (Exception e) {
